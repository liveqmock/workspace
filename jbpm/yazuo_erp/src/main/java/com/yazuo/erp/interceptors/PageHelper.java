...
public class PageHelper implements Interceptor {
    ...
    private static String sqlSegment = "@@"; //当存在参数withJoin时， 需要替换掉的sql语句片段字符
     ...
     * @param pageSize
     * @param calcute count 
     * @param withJoin - 是否存在 关联查询 （mybatis存在Collection映射）
     */
    public static void startPage(int pageNum, int pageSize, boolean count, boolean withJoin) {
    	localPage.set(new Page(pageNum, pageSize, count? Page.SQL_COUNT: Page.NO_SQL_COUNT, withJoin));
    }
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (localPage.get() == null && rowBounds == RowBounds.DEFAULT) {
            ...
            if (page.getTotal() > Page.NO_SQL_COUNT) {
                //求count - 重写sql
                msObject.setValue("sqlSource.boundSql.sql", getCountSql(sql, page.getWithJoin()));
                //查询总数
     ...
     * @return
     */
    private String getCountSql(String sql, boolean withJoin) {
    	if(withJoin){
    	  String result = "select count(1) from  (select alais_id from (" + sql.replace(sqlSegment,"") + ") as t_for_group_by GROUP BY alais_id) as t_result";
//    	  System.out.println(result);
    	  return result; 
//    	  return "select count(distinct(tmp_count.alais_id)) from (" + sql.replace(sqlSegment,"") + ") as tmp_count";
    	}else{
    ...
    private String getPageSql(String sql, Page page) {
        ...
        if("postgresql".equals(dialect)){
        	if(page.getWithJoin()){
        		pageSql.append(sql.replace(sqlSegment, " limit "+page.getPageSize()+" offset "+page.getStartRow()));
        	}else{
        		pageSql.append(sql);
