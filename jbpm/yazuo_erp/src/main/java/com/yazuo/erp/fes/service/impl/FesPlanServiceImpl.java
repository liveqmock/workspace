...
public class FesPlanServiceImpl implements FesPlanService {
	 ...
	 * @throws
	 */
	public void checkMonthlyPlan(Integer userId, Integer merchantId, String planItemType, Integer planId) {
		if ("1".equals(planItemType)) {
			Map<String, Object> inputMap = new HashMap<String, Object>();
			inputMap.put("userId", userId);
			inputMap.put("month", DateUtil.format(new Date(), "yyyy-MM"));
			inputMap.put("merchantId", merchantId);
			inputMap.put("planId", planId);
			int count = this.fesPlanDao.getCountOfMonthlyPlans(inputMap);
			if (count > 0) {
				throw new FesBizException("每月只能创建一次月报讲解的工作计划");
			}
		}
	}
    ...
    public FesPlanVO getMonthlyPlanVO(Integer merchantId, Date from, Date to) {
        ...
        if (fesPlanVOs.size() > 1) {
            throw new FesBizException("月报发送内容数量多于1");
        }else {
            return fesPlanVOs.size() == 1 ? fesPlanVOs.get(0) : null;
        }
    }
	...
	public int updateCompleteFesPlanById(Map<String, Object> paramMap) {
		...
		// 校验，月报讲解一个月只能完成一次
		this.checkCompleteMonthlyPlan(userId, planId, plan.getMerchantId(), plan.getPlanItemType());
	 ...
	 * @param merchantId
	 * @param planItemType
	 * @see com.yazuo.erp.fes.service.FesPlanService#checkCompleteMonthlyPlan(java.lang.Integer, java.lang.Integer, java.lang.Integer, java.lang.String)
	 */
	public void checkCompleteMonthlyPlan(Integer userId, Integer planId, Integer merchantId, String planItemType) {
		if ("1".equals(planItemType)) {
			Map<String, Object> inputMap = new HashMap<String, Object>();
			inputMap.put("userId", userId);
			inputMap.put("month", DateUtil.format(new Date(), "yyyy-MM"));
			...
			if (count > 0) {
				throw new FesBizException("此商户本月的月报回访已经完成，不能重复完成");
			}
		}
	}
