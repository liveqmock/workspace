...
//		}
		...
		if("4".equals(documentType)){  //替换 月报讲解时间
			replacePlanEndTime(sysQuestion, sysQuestionList);
		}
	 ...
	 *  替换 月报讲解时间
	 */
	private void replacePlanEndTime(SysQuestionVO sysQuestion, List<Map<String, Object>> sysQuestionList) {
			Integer merchantId = sysQuestion.getMerchantId();
			assertNotNull("商户ID能为空!", merchantId);
			List<FesPlanVO> fesPlanForReq = this.besRequirementService.getFesPlanForReq(merchantId);
