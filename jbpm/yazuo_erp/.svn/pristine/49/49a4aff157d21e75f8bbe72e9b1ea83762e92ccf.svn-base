    ...
    util = _.extend(util, {
                ...
                xhr.done(function (responseData) {
                    if (!responseData.flag) {   //错误统一处理
                        if (responseData.timeout) { //超时自动跳到登录页
                            location.href = erp.BASE_PATH + 'login.jsp';
                        } else {
                            util.alert({
                    ...
                    _.each(flatRouteData(rows), function (row) {
                        ...
                        mmRouter.router.get(route, function () {
                            ...
                            if (!deps) {    //对于不缓存的page，第二次加载时依赖已经准备好了
                                //替换true标志,补全路径前缀
                                if (erp.PUBLISH_MODEL === "development") {
                                    css = _.map(css, function (itemPath) {
                                        if (itemPath === true) {
                                            return 'css!' + erp.PAGE_PATH + path + '/' + fileName + '.css';
                                        } else {
                                            return 'css!' + erp.ORIGIN_PATH + itemPath;
                                        }
                                    });
                                } else {
                                    css = [];   //已提前加载
                                ...
                                js = _.map(js, function (itemPath) {
                                    var realPath;
                                    if (itemPath === true) {
                                        realPath = erp.PAGE_PATH + path + '/' + fileName + '.js';
                                    } else {
                                        realPath = erp.ORIGIN_PATH + itemPath;
                                    }
                                    if (erp.PUBLISH_MODEL === "development") {
                                        return realPath;
                                    } else if (erp.PUBLISH_MODEL === "product") {
                                        return realPath.slice(erp.PORTAL_PATH.length).replace(/[\.|\\|/|_|-]/g, '');
                                    }
                                });
                                //确定html的加载路径，支持页面模板复用
                                if (tmpl) {
                                    if (tmpl === true) {
                                        htmlPath = 'text!' + erp.PAGE_PATH + path + '/' + fileName + '.html';
                                    } else {
                                        htmlPath = 'text!' + erp.ORIGIN_PATH + tmpl;
                                    }
                                } else {
                                    htmlPath = 'text!' + erp.PAGE_PATH + path + '/' + fileName + '.html';
                                }
        ...
        "alert": function (opts) {
            ...
            model = {
                ...
                "onOpen": function () {
                    this.setContent('<div class="fn-clear"><i class="icon-tip icon-' + opts.iconType + ' fn-left">' + iconChar + '</i>&nbsp;&nbsp;<span class="tip-content">' + opts.content + '</span></div>', true);
                },
                "onClose": function () {
        ...
        "adjustModuleCssUrl": function (cssText, relPath) {
            ...
            var moduleBasePath,
                publishModel = erp.PUBLISH_MODEL;
            moduleBasePath = erp.MODULE_PATH;
        ...
        "hasPermission": function (permissionName) {
            var permissionData = erp.getAppData('permission');
            return _.some(permissionData, function (itemData) {
                return itemData && (itemData.remark === permissionName);
            });
        },
        /**
         * 手动跳转页面，可记录hash导航
        ...
        "testing": function (eles,jqObj) {//入口方法，业务层调用（eles为dom对象）
            ...
            testEles.each(function(index,item){//循环遍历
                ...
                $(item).on('blur',function(){util.testStart(this)});//注册onblur事件
                if(!flag){
                    isThrough = false;
                }
            });
