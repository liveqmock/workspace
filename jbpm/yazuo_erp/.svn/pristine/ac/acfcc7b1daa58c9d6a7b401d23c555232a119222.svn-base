...
//		String message = mktContactVO.getName()+ honorific +"你好,您商户的实体卡已配送（快递单："+fesStowage.getLogisticsNum()+"），请注意查收。";
		...
		String message = mktContactVO.getName()+ honorific +"你好,您商户的设备("+storageInfo+") 已配送(快递单："+fesStowage.getLogisticsNum()+"),请注意查收。";
		if(fesStowage.getDistributeFlag().equals(0)){//实体卡配送
//			短信内容：某先生/女士您好，您商户的实体卡已配送（快递单：************），请注意查收。【雅座在线】
			message = mktContactVO.getName()+ honorific +"你好,您商户的实体卡已配送（快递单："+fesStowage.getLogisticsNum()+"）,请注意查收。";
		}
		LOG.info(message);
