 ...
 */
@Controller
@RequestMapping({"/activity", "fesActivity"})
public class FesActivityController {
	...
	@ResponseBody
	public JsonResult updateActivityStatus(Integer activityId, String status,@RequestParam(value="reason", required=false) String reason,
	...
	@ResponseBody
	public JsonResult listRemind(@RequestBody SysRemindVO sysRemind, HttpServletRequest request) {
		SysUserVO user = (SysUserVO)request.getSession().getAttribute(Constant.SESSION_USER);
		 //分页
		 PageHelper.startPage(sysRemind.getPageNumber(), sysRemind.getPageSize(), true);
		 
		 Page<SysRemindVO> listMap = sysRemindService.getSysReminds(sysRemind, user);
