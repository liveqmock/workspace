...
public class MktContactServiceImpl implements MktContactService {
	 ...
	 */
	@Override
	public List<Map<String, Object>> getContactsDropDownList(MktContactVO mktContact){
		...
		CollectionUtils.collect(mktContactsMap.iterator(), new Transformer() {
			@Override
	...
	@Override
	public List<MktContactVO> getContactForMerchantRoles(List<String> roleTypes, Integer merchantId) {
		if (roleTypes.size() > 0) {
			return this.mktContactDao.getContactForMerchantRoles(roleTypes, merchantId);
		}else {
	 ...
	 */
	@Override
	public MktContactVO updateMktContactForDelete(int id) {
		MktContactVO mktContact = new MktContactVO();
		mktContact.setId(id);
		mktContact.setIsEnable("0");
		mktContactDao.updateMktContact(mktContact );
		return mktContact;
	}
}
