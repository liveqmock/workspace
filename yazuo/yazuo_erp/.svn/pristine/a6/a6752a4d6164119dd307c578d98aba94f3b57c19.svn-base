...
import com.yazuo.erp.bes.service.impl.BesRequirementServiceImpl;
...
//		System.out.println("method invoke!");
	...
	private static final Log LOG = LogFactory.getLog(SysDictionaryServiceImpl.class);
	/**
	 * 标准方法,保存数据字典行
	 * @param attributeName{0:数据库字段属性, 1:对象属性}
	 */
	@Override
	public void setStdDicRow(Object vo, String dictionaryType, String... attributeName) {
		String dicKey;
		try {
			dicKey = (String)MethodUtils.invokeMethod(vo, "get"+StringUtils.capitalize(attributeName[0]), null);
			if(StringUtils.isEmpty(dicKey)){
				LOG.warn("字段: "+attributeName[0]+"不应该为空!");
			}else{
				Map<String, Object> row = this.getSysDictionaryMapByTypeAndKey(dictionaryType, dicKey);
				MethodUtils.invokeMethod(vo, "set"+StringUtils.capitalize(attributeName[1]), row);
			}
		} catch (Exception e) {
