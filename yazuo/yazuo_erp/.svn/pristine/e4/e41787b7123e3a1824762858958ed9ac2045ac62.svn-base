...
public class FesReportServiceImpl implements FesReportService {
	...
	public boolean exportReport(Map<String, Object> paramMap, HttpServletRequest request, HttpServletResponse response)
		...
		if (null == startTimeObj) {
			throw new FesBizException("请输入查询开始时间");
		}
		Date startTime = DateUtil.toDateFromMillisecond(startTimeObj);

		Object endTimeObj = paramMap.get("endTime");
		if (null == endTimeObj) {
			throw new FesBizException("请输入查询结束时间");
		}
		Date endTime = DateUtil.toDateFromMillisecond(endTimeObj);

		String sheetName = "运营报告";

		String start = DateUtil.format(startTime, "yyyy-MM-dd");
		String end = DateUtil.format(endTime, "yyyy-MM-dd");

		// 查询CRM商户统计数据
		Map<String, Object> crmMap = this.queryStatisticsOfCrm(start, end);
	 ...
	 * @throws
	 */
	private Map<String, Object> queryStatisticsOfCrm(String startDate, String endDate) {
		// 调用CRM接口，查询报表数据
		Map<String, Object> crmMap = new HashMap<String, Object>();
		try {
			// 调用CRM查询商户报表信息接口，查询数据
			String url = erpstatUrl + "/startdate/" + startDate + "/enddate/" + endDate;
