...
public class BesMonthlyReportServiceImpl implements BesMonthlyReportService {
	...
	public void checkIfCompletedWorkPlan() {
		...
		for (FesPlanVO fesPlanVO : fesPlans) {
            ...
            if (fesPlanVO.getStatus().equals("4")) { // 已经完成
            	//如果不存在需求,创建需求
                this.createBesReqIfNotExists(fromMonth, toMonth, fesPlanVO, merchantId);
                ...
                if (!existMonthlyReport && startTime.before(calendar.getTime())) { //不存在 且 已过期
                    ...
                    this.besRequirementService.saveCreateBesReq(fesPlanVO);
                	//如果不存在需求,创建需求
                    this.createBesReqIfNotExists(fromMonth, toMonth, fesPlanVO, merchantId);
                }
            }
	 ...
	 * //如果不存在需求,创建需求
	 */
	private void createBesReqIfNotExists(Date fromMonth, Date toMonth, FesPlanVO fesPlanVO, final Integer merchantId) {
		//判断是否已经存在 回访需求  目前一个商户一个月只有一个月报回访需求, 将来扩展
	...
	private void saveCreateBesReq(Date now, final Integer merchantId, final Integer contactId) { 
        ...
        String month = fdf.format(DateUtils.addMonths(now, -1));
		...
		besRequirement.setProcessStatus("b_1");
	...
	public boolean saveSendMonthlyRpt(Integer merchantId, SysUserVO sessionUser){
		...
		besMonthlyReport.setMerchantId(merchantId);
		Date fromMonth = DateUtil.fromMonth();
        Date lastDay = DateUtils.addMonths(DateUtils.truncate(fromMonth, Calendar.MONTH), 1);
        boolean existMonthlyReport = this.existMonthlyReport(merchantId, explainAccessFactor, fromMonth, lastDay);
		if(!existMonthlyReport){
            ...
            BesMonthlyReportVO monthlyReportVO = this.getMonthlyReport(merchantId, explainAccessFactor, fromMonth, lastDay);
            FesPlanVO fesPlanVO = this.fesPlanService.getMonthlyPlanVO(merchantId, fromMonth, lastDay);
            //"请先创建商户该月的月报讲解工作计划，再发送月报!"
            if(fesPlanVO == null) return false;
