...
public class GpStatisticsReportDaoImpl implements GpStatisticsReportDao {
	...
	public Map<String, Object> getMemberDiskNum(Integer brandId, String month, Integer indexId, BigDecimal proportion) {
		...
		List<Integer> list1 = jdbcTemplateCrm210Trade.queryForList("SELECT merchant_id FROM trade.merchant AS mer1 " +
				"WHERE status = 1", Integer.class);
		
		String sql = "SELECT COALESCE(sum(? * index_value),0) AS index_value  " +
				"FROM public.merchant_index_plan AS plan where index_id = ? AND month = ? AND plan.brand_id=?";
		...
		if(list1!=null&&list1.size()>0){
			...
			sql= sql+ " AND merchant_id in "+str;
		}
