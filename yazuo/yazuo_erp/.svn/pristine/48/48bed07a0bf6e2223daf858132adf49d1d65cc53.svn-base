...
public class SysUserMerchantServiceImpl implements SysUserMerchantService {
	...
	public int saveSysUserMerchant(SysUserMerchantVO sysUserMerchant, SysUserVO sessionUser) {
		...
		if (userMerchantId == null) {
			...
			synMerchant.setServiceStartTime(new Date());
	 ...
	 */
	@Override
	public Integer getUserMerchantId(SysUserMerchantVO sysUserMerchant) {
	...
	@Resource private SysSalesmanMerchantService sysSalesmanMerchantService;
	@Resource private MktFormerSalesMerchantService mktFormerSalesMerchantService;

	@Override
	public void saveSalesAndFrontEndPersion(SynMerchantVO synMerchantVO, SysUserVO sessionUser) {
		//更新前端负责人
		Integer merchantId = synMerchantVO.getMerchantId();
		if(synMerchantVO.getInputUserId()!=null){
			SysUserMerchantVO sysUserMerchant = new SysUserMerchantVO();
			sysUserMerchant.setUserId(synMerchantVO.getInputUserId());
			sysUserMerchant.setMerchantId(merchantId);
			//查找是否存在商户前端负责人
			SysUserMerchantVO sysUserMerchantVO = new SysUserMerchantVO();
			sysUserMerchantVO.setMerchantId(sysUserMerchant.getMerchantId());
			...
			if(sysUserMerchants.size()<=0) {
				...
				sysUserMerchant.setId(sysUserMerchants.get(0).getId());// 主键
				sysUserMerchantDao.updateSysUserMerchant(sysUserMerchant);
			}
		}
		//更新销售负责人并保存历史销售负责人
		final SysSalesmanMerchantVO salesMan = this.sysSalesmanMerchantService.getSysSalesmanMerchantByMerchantId(merchantId);
		if (salesMan != null && !salesMan.getUserId().equals(synMerchantVO.getSalesId())) { 
			mktFormerSalesMerchantService.saveSalesMan(sessionUser, merchantId, synMerchantVO.getSalesId(), salesMan);
		} 
	}
}
