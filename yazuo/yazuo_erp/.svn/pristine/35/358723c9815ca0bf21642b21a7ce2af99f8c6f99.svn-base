...
public class BesMonthlyCheckServiceImpl implements BesMonthlyCheckService {
	...
	@Override
	public boolean monthlyReportUncompleted(Integer merchantId) {
		if(merchantId!=null){
			BesMonthlyCheckVO besMonthlyCheck = new BesMonthlyCheckVO();
			besMonthlyCheck.setMerchantId(merchantId);
			besMonthlyCheck.setSortColumns("update_time desc");
			List<BesMonthlyCheckVO> besMonthlyChecks = this.getBesMonthlyChecks(besMonthlyCheck);
			if(besMonthlyChecks.size()>0){
				BesMonthlyCheckVO besMonthlyCheckVO = besMonthlyChecks.get(0);
				if(besMonthlyCheckVO.getMonthlyCheckState().equals("1"))
					return true; //已完成
			}
		}
		return false;
