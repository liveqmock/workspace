...
//		return FileUploaderUtil.uploadFile(myfiles, documentFileTempPath, null, 0, request);
    ...
    @Override
    public SysDocumentVO getDocumentByMerchantAndType(Integer merchantId, String documentType, Date from, Date to) {
        SysDocumentVO documentVO = new SysDocumentVO();
        documentVO.setMerchantId(merchantId);
        documentVO.setDocumentType(documentType);
        documentVO.setBeginInsertTime(from);
        documentVO.setEndInsertTime(to);
        documentVO.setSortColumns("id desc");
        List<SysDocumentVO> documentVOs = this.getSysDocuments(documentVO);
        return documentVOs.size() > 0 ? documentVOs.get(0) : null;
    }
	...
	public SysDocumentVO querySysDocumentById(SysDocumentVO sysDocumentVO) {
        ...
        sysDocumentDtl.setSortColumns(" question_id ");
