...
public class SynMerchantController {
	 ...
	 *  上线，服务开始时间、服务年限、门店赠送月份、上线状态、销售人员、前端顾问(如果是上线状态则必填)。这些信息都必填。
	 */
	...
	public JsonResult updateSynMerchantForOnline(@RequestBody SynMerchantVO synMerchantVO,
		...
		if ("1".equals(synMerchantById.getMerchantStatus())) { // 原状态为已上线，只修改基本信息
			...
			if("1".equals(synMerchantVO.getMerchantStatus())){//如果更改成已上线，结束代办事项
	...
	public JsonResult getSynMerchantsForAment(@RequestBody SynMerchantVO synMerchantVO) {
		...
		assertNotNull("页号不允许为空!", synMerchantVO.getPageNumber());
		assertNotNull("页总数不允许为空!", synMerchantVO.getPageSize());
		PageHelper.startPage(synMerchantVO.getPageNumber(), synMerchantVO.getPageSize(), true);
		Page<SynMerchantVO> pageList = (Page<SynMerchantVO>)synMerchantService.getSynMerchants(synMerchantVO);
		Map<String, Object> dataMap = new HashMap<String, Object>();
