 ...
 * 试题选择组件
 */
define(["avalon", "util", "json", "text!./qselector.html", "text!./qselector.css", "common", "../../widget/dialog/dialog"], function(avalon, util, JSON, tmpl, cssText) {
    var win = this,
        erp = win.erp;
    ...
    if (!styleData["qselector"]) {
        try {
            styleEl.html(styleEl.html() + util.adjustModuleCssUrl(cssText, 'qselector/'));
        } catch (e) {
            styleEl[0].styleSheet.cssText += util.adjustModuleCssUrl(cssText, 'qselector/');
        }
        styleData["qselector"] = true;
        styleEl.data('module', styleData);
    }
    var module = erp.module.qselector = function(element, data, vmodels) {
        var opts = data.qselectorOptions,
            dialogId = data.qselectorId + '-dialog',
            elEl = $(element);
        var vmodel = avalon.define(data.qselectorId, function (vm) {
            avalon.mix(vm, opts);
            ...
            vm.$questionOpts = {
                ...
                "updateList": function () {
                    ...
                    requestData = vm.onListRequestData.call(vm, requestData);
                    util.c2s({
                        "url": erp.BASE_PATH + vm.listPath,
    ...
    module.defaults = {
        "title": "",
        "multi": false,  //默认支持单选
        "listPath": "rule/getQuestionByCoursewareId.do",
