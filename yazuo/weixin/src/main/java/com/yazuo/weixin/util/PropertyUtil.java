...
public class PropertyUtil
{
	...
	/**
	 * 
	 *是否每次都重新加载一次配置文件，isSingleton为true表示否，反之则是.<br/>
	 *
	 * User: huijun.zheng<br/>
	 * Date: 2013-12-25<br/>
	 * Time: 下午4:25:24<br/>
	 ...
	 * @throws IOException<br/>
	 */
	public synchronized Properties getProperties(String propertiesFileName, boolean isSingleton) throws IOException
	{
		if (propertiesFileName == null || "".equals(propertiesFileName)) throw new IllegalArgumentException(
				"The propertiesFileName is illegal argument!");
		...
		{	
			...
			if (prop == null)
			{
				...
				if (is == null) throw new FileNotFoundException("The file \"" + propertiesFileName
						+ "\" is  not found!");
				try
				{
					prop.load(is);
		...
		else// 每次读取属性时都从配置文件进行，从新读取
		{
			prop = new Properties();
			InputStream is = this.getClass().getResourceAsStream(propertiesFileName);
			if (is == null) throw new FileNotFoundException("The file \"" + propertiesFileName + "\" is  not found!");
			try
			{
				prop.load(is);
			}
	 ...
	 * @param propertiesFileName
	 * @param propertyName
	 * @param isSingleton false表示每次读取属性值，从重新加载一次配置文件，true则仅仅加载一次
	 * @return
	 * @throws IOException<br/>
	 */
	public String getProperty(String propertiesFileName, String propertyName,boolean isSingleton) throws IOException
	{
