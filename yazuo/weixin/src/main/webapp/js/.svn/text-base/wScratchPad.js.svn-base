...
(function($)
{
	...
	{
		width			: 225,					// set width - best to match image width
		height			: 108,					// set height - best to match image height
		image			: '../image/ggkShade.png',	// set image path
	...
	ScratchPad.prototype = 
	{
		...
		scratchFunc: function(e, $this, event)
		{
			...
			var data=$this.ctx.getImageData(0,0,225,108).data;
			...
			if(j<=225*108*0.3){//表示刮开70%
				if(flag == 0) {
					...
					var re = $("#pafrequency").text();
					var newre = re-1;
					jQuery("#pafrequency").html(newre);
					flag =1;
				} 
			}
