...
<script language="javascript">
	...
	$('#getIdentifyNO').live('click', function() {
		...
		$.ajax({
		 type : "post",
		 url : '<%=path%>/weixin/phonePage/registerIdentifyingCode.do',
		 cache : false,
		 dataType : "text",
		 data : "brandId="+"<%=business.getBrandId()%>"+"&weixinId="+"<%=weixinId%>"+"&title="+"<%=title%>"+"&phoneNo="+$('#phoneNo').val().trim()+"&random="+Math.random(),
	...
	function saveMember(){
		...
		$.ajax({
			...
			+"&secCode="+$('#secCode').val().trim()+"&userPwd="+$('#userPwd').val().trim()+"&phoneNo="+$('#phoneNo').val().trim()+"&identifyCode="+identifyNo
			+"&brandId="+"<%=brandId%>"+"&weixinId="+"<%=weixinId%>"+"&random="+Math.random(),
			success:function(data){
				...
				if(json.trim()=="success"){
					...
					$('.mem-submit').show();
					//$('#getIdentifyNO').show();
					$('.error-mesg').text('验证码错误');
				}else if(json.trim()=="identifyExpire"){
					$('.error-mesg').text('');
					$('.mem-submit').show();
					//$('#getIdentifyNO').show();
					$('.error-mesg').text('验证码过期,重新获取');
				}else if(json.trim()=="3"){
					$('.error-mesg').text('');
					$('.mem-submit').show();
					//$('#getIdentifyNO').show();
...
<body>
	<div class="w">
		...
		<div class="mem-succ">
			<img src="<%=path%>/images/suc1.png" width="40" /><span id="cccc">绑定实体卡成功</span>
		</div>
		<div class="sub-div">
			<a href="#" class="mem-sub" id="submitSuccess">进入会员中心</a>
		</div>
		</section>
	</div>
</body>
